...
class PluginManager:
    ...
    def _modify_block_string(current_str: str, module: str, add: bool) -> str:
        ...
        if add:
            if module not in items:
                items.append(module)
        elif module in items:
            items.remove(module)
        return CommonUtils.convert_module_format(items)

    @classmethod
    async def _calculate_affected_groups(
        cls, target_groups: set[str], status: bool, is_whitelist_mode: bool, bot: Bot
        ...
        if is_whitelist_mode and status:
            active_groups = await PlatformUtils.get_group_list(bot, only_group=True)
    ...
    async def batch_update_status(
        ...
        for gid in groups_to_open | groups_to_close:
            ...
            await GroupConsole.get_or_create(
                group_id=gid, defaults={"platform": platform}, bot_id=bot.self_id
            )

        groups_obj = await GroupConsole.filter(
            group_id__in=list(affected_ids), bot_id=bot.self_id
        ).all()
        update_list = []
        ...
        elif is_superuser and use_su_field:
    ...
    async def set_all_plugin_status(
        ...
        if group_id:
            if group := await GroupConsole.get_group(bot_id=bot_id, group_id=group_id):
                norm_field = strategy.norm_field
                su_field = strategy.su_field
        ...
        if entity:
            if group_id:
                is_su_blocked, _ = await strategy.check_block_status(
                    bot_id=bot.self_id, group_id=group_id, module=entity.module
                )
    ...
    async def batch_set_group_active_status(
        ...
        if groups_to_wake:
            await GroupConsole.filter(
                group_id__in=list(groups_to_wake), bot_id=bot.self_id
            ).update(status=True)
        if groups_to_sleep:
            await GroupConsole.filter(
                group_id__in=list(groups_to_sleep), bot_id=bot.self_id
            ).update(status=False)
